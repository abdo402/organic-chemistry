...
(function(){
  ...
  function init(){
    resize();pts=[];
    for(let i=0;i<30;i++)pts.push({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-.5)*.35,vy:(Math.random()-.5)*.35,r:Math.random()*1.8+.6});
  }
  function draw(){
...
const CHAPTERS=[
  {
    label:'Organic',
    ids:['chk0','chkCalc','chk1','chk2','chk-alkiso','chk3','chk4b','chkOR','chkPoly','chkAromatic','chkHCDerivatives','chkCyclic','chkAlcoholsPhenols','chkSynth'],
    names:['Hydrocarbons','Calc','IUPAC','Isomers','Iso Builder','Library','Func. Groups','Org. Reactions','Polymers','Aromatic/Alkynes','HC Derivatives','Cyclic HC','Alcohols/Phenols','Synthesis']
  },
  {
    label:'General',
...
const SEARCH_INDEX=[
  ...
  {title:'Aromatic Chemistry & Alkynes',ctx:'Benzene aromaticity Hückel rule delocalized pi electrons EAS electrophilic aromatic substitution nitration halogenation sulfonation Friedel-Crafts directing effects ortho para meta NAS Meisenheimer alkyne sp hybridised triple bond ethyne acetylene terminal acidic hydrogenation Lindlar Kucherov hydration cyclic trimerisation carbylamine',anchor:'aromatic-section'},
  {title:'Hydrocarbon Derivatives',ctx:'Haloalkanes SN1 SN2 elimination E2 Walden inversion carbocation Zaitsev alcohol oxidation esterification dehydration Fischer aldehyde ketone Tollens Fehling Brady 2,4-DNPH cyanohydrin carboxylic acid saponification ester hydrolysis amine diazotisation diazonium azo amide nitrile chain extension functional group reactions',anchor:'hydrocarbon-derivatives'},
  {title:'Cyclic Hydrocarbons',ctx:'Cycloalkane cycloalkene cyclopropane cyclobutane cyclopentane cyclohexane ring strain Baeyer angle strain chair boat conformation axial equatorial 1,3-diaxial torsional transannular ring-opening addition combustion radical substitution hydrogenation degrees of unsaturation',anchor:'cyclic-hydrocarbons'},
  {title:'Alcohols & Phenols',ctx:'Alcohol primary secondary tertiary hydroxyl OH IUPAC nomenclature ethanol methanol propanol butanol diol triol glycerol Lucas test ZnCl2 sodium alkoxide oxidation K2Cr2O7 dehydration esterification phenol aromatic ring acidic pKa NaOH sodium phenoxide bromination tribromophenol FeCl3 violet nitration picric acid Kolbe-Schmitt salicylic acid aspirin',anchor:'alcohols-phenols'},
